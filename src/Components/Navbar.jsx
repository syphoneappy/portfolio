...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsSmallScreen(window.innerWidth < 768); // Adjust this width as needed for small screen detection
    };
  ...
  return (
    <div className='flex justify-between items-center w-full h20 text-white bg-black px-4 fixed '>
      ...
      <ul className={`hidden ${isSmallScreen ? 'md:hidden' : 'md:flex'} justify-center items-center`}>
        {links.map(({ id, link }) => (
